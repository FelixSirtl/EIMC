...
namespace QL4BIMinterpreter.QL4BIM
{
    ...
    public class QLEntites : IEnumerable<QLEntity>
    {
        private readonly QLEntity[] entities;

        public int Length => entities.Length;

        // Declare an array to store the data elements.
        public QLEntites(QLEntity[] entities)
        {
            this.entities = entities;
        }

        public QLEntity[] ToArray(){
            return new List<QLEntity>(entities).ToArray();
        }

        // Define the indexer to allow client code to use [] notation.
        public QLEntity this[int i]
        {
            get {
                if (i >= 0) 
                    return entities[i];
                else
                    return entities[entities.Length + i];
            }
            ...
            set { 
                if (i >= 0) 
                    ...
                    entities[entities.Length + i]  = value;
                }
        }

        public IEnumerator<QLEntity> GetEnumerator()
        {   
