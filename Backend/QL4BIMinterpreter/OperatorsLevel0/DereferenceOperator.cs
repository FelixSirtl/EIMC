...
namespace QL4BIMinterpreter.OperatorsLevel0
{
    public class DereferenceOperator : IDereferenceOperator
    {
        ...
        public List<QLEntites> ResolveReferenceTuplesIn(IEnumerable<QLEntites> tuples, int attributeIndex, bool replace, string referenceName)
        {
            var result = new List<QLEntites>();
        ...
        private QLEntites AddEntityReplace(QLEntites tuple, QLEntity referencedEntity)
        {
            ...
            return new QLEntites(newTuple.ToArray());
        }

        private QLEntites AddEntity(QLEntites tuple, QLEntity referencedEntity)
        {
        ...
        public IEnumerable<QLEntites> ResolveReferenceSetIn(IEnumerable<QLEntity> entites, string referenceName)
        {
            var result = new List<QLEntites>();
            foreach (var qlEntity in entites)
            {
                ...
                var referencedEntities = EntitesFromRefs(referenceValue);
                result.AddRange( referencedEntities.Select(r => new QLEntites(new QLEntity[] { qlEntity, r })));
            }
