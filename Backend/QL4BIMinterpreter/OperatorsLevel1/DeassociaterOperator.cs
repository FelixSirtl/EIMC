...
namespace QL4BIMinterpreter.OperatorsLevel1
{
    class DeassociaterOperator : IDeassociaterOperator
    {
        ...
        public IEnumerable<QLEntites> GetTuplesRelAtt(IEnumerable<QLEntites> tuples, int attributeIndex, string[] exAtts)
        {
            var firstPairs = dereferenceOperator.ResolveReferenceTuplesIn(tuples, attributeIndex, false, exAtts[0]);

            if (firstPairs.Count == 0)
                return new List<QLEntites>();
        ...
        public IEnumerable<QLEntites> GetTuplesSet(IEnumerable<QLEntity> entites, string[] exAtts)
        {
